...
import {
    Container, Box, Typography, Paper, Chip, Stack, IconButton,
    List, ListItem, ListItemAvatar, Avatar, ListItemText, Fab, Tabs, Tab, Divider, CircularProgress
} from '@mui/material';
import {
    ArrowBack, Edit, WaterDrop,
    PhotoLibrary, Info, QrCodeScanner, Cake,
    Female, Male, Store, Person, Badge, LocalOffer,
    MonitorWeight
} from '@mui/icons-material';
...
const CowProfile: React.FC = () => {
    ...
    const navigate = useNavigate();
    const [tabValue, setTabValue] = useState(1); // Default to Data tab
    ...
    useEffect(() => {
        ...
        // Find by ID (checking both string and number comparison)
        const foundCow = storedCows.find((c: any) => c.id.toString() === id);
    ...
    if (!cow) {
        return (
            <Container sx={{ mt: 4, textAlign: 'center' }}>
                ...
                <IconButton onClick={() => navigate(-1)}><ArrowBack /> Back</IconButton>
            </Container>
        );
    }
    ...
    return (
        <Container maxWidth="sm" sx={{ p: 0, pb: 10 }}>
            ...
            <Box sx={{ position: 'relative', height: 240, bgcolor: '#eee' }}>
                ...
                <img
                    src={cow.photos?.muzzle || "https://placehold.co/600x400?text=No+Image"}
                    alt={cow.name}
                    style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                />
                <Box sx={{
                    ...
                    <Typography variant="h4" fontWeight={800}>{cow.name || 'Unnamed'}</Typography>
                ...
                <Paper elevation={3} sx={{ borderRadius: 3, overflow: 'hidden', mb: 3 }}>
                    ...
                    {tabValue === 0 && (
                        <Box sx={{ p: 2 }}>
                            ...
                            <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 2 }}>
                                {Object.entries(cow.photos || {}).map(([key, url]: any) => (
                                    <Box key={key}>
                                        <Paper elevation={0} sx={{
                                            height: 120, bgcolor: '#f5f5f5', borderRadius: 2, overflow: 'hidden', position: 'relative',
                                            border: '1px solid #eee'
                                            ...
                                            <Typography variant="caption" sx={{
                                                position: 'absolute', bottom: 0, left: 0, right: 0,
                                                bgcolor: 'rgba(0,0,0,0.6)', color: 'white', p: 0.5, textAlign: 'center', textTransform: 'capitalize'
                                            }}>
                                                {key}
                                            </Typography>
                                        </Paper>
                    ...
                    {tabValue === 1 && (
                        <Box sx={{ p: 2 }}>
                            {/* STATUS BADGES */}
                            <Stack direction="row" spacing={1} sx={{ mb: 3 }}>
                                <Chip label={cow.productionStatus || 'Unknown'} color="success" size="small" icon={<WaterDrop />} />
                                <Chip label={cow.species} variant="outlined" size="small" color="info" />
                            </Stack>

                            {/* SECTION 1: GENERAL / OWNER (Assuming static or generic for now based on your schema) */}
                            <Typography variant="overline" color="text.secondary" fontWeight="bold">REGISTRATION INFO</Typography>
                            <List dense disablePadding sx={{ mb: 3, bgcolor: '#f9fafb', borderRadius: 2, p: 1 }}>
                                <ListItem disableGutters>
                                    <ListItemAvatar><Avatar sx={{ width: 28, height: 28 }}><Person sx={{ fontSize: 18 }} /></Avatar></ListItemAvatar>
                                    <ListItemText primary="Registration Date" secondary={new Date(cow.createdAt).toLocaleDateString()} />
                                </ListItem>
                                <ListItem disableGutters>
                                    <ListItemAvatar><Avatar sx={{ width: 28, height: 28 }}><Badge sx={{ fontSize: 18 }} /></Avatar></ListItemAvatar>
                                    <ListItemText primary="System ID" secondary={cow.id} />
                                </ListItem>
                            </List>
                            ...
                            <List dense>
                                <ListItem disablePadding sx={{ mb: 1 }}>
                                    <ListItemAvatar><Avatar sx={{ bgcolor: 'primary.light', color: 'primary.main' }}><QrCodeScanner /></Avatar></ListItemAvatar>
                                    <ListItemText primary="Identity" secondary={`Tag: ${cow.tagNumber}`} />
                                </ListItem>
                                <ListItem disablePadding sx={{ mb: 1 }}>
                                    <ListItemAvatar>
                                        <Avatar sx={{ bgcolor: 'info.light', color: 'info.main' }}>
                                            {cow.sex === 'Female' ? <Female /> : <Male />}
                                        </Avatar>
                                    </ListItemAvatar>
                                    <ListItemText primary="Species / Sex" secondary={`${cow.species} / ${cow.sex}`} />
                                </ListItem>
                                <ListItem disablePadding sx={{ mb: 1 }}>
                                    <ListItemAvatar><Avatar sx={{ bgcolor: 'warning.light', color: 'warning.main' }}><LocalOffer /></Avatar></ListItemAvatar>
                                    <ListItemText primary="Breed" secondary={cow.breed} />
                                </ListItem>
                                <ListItem disablePadding sx={{ mb: 1 }}>
                                    <ListItemAvatar><Avatar sx={{ bgcolor: 'secondary.light', color: 'secondary.main' }}><Cake /></Avatar></ListItemAvatar>
                                    <ListItemText primary="Age & DOB" secondary={`${calculateAge(cow.dob)} (${cow.dob})`} />
                                </ListItem>
                                <ListItem disablePadding sx={{ mb: 1 }}>
                                    <ListItemAvatar><Avatar sx={{ bgcolor: 'success.light', color: 'success.main' }}><Store /></Avatar></ListItemAvatar>
                                    <ListItemText primary="Source" secondary={cow.source} />
                                </ListItem>
                            </List>
                            ...
                            <List dense>
                                <ListItem disablePadding sx={{ mb: 1 }}>
                                    <ListItemAvatar><Avatar sx={{ bgcolor: 'error.light', color: 'error.main' }}><SchemaIcon /></Avatar></ListItemAvatar>
                                    <ListItemText primary="Lineage (Sire / Dam)" secondary={`${cow.sireTag || 'Unknown'} / ${cow.damTag || 'Unknown'}`} />
                                </ListItem>
                                <ListItem disablePadding sx={{ mb: 1 }}>
                                    <ListItemAvatar><Avatar sx={{ bgcolor: 'primary.light', color: 'primary.main' }}><MonitorWeight /></Avatar></ListItemAvatar>
                                    <ListItemText primary="Current Stats" secondary={`Weight: ${cow.currentWeight || 'Not Recorded'}`} />
                                </ListItem>
                            </List>
                        </Box>
                    )}
